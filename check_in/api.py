...
from frappe.utils import now_datetime, nowdate, get_link_to_form
import json
...
def get_employee(attendance_id="123"):
	...
	try:
		...
		employee,name,company,image =frappe.db.get_value("Employee",{'attendance_device_id':attendance_id}, ["name", "employee_name","company","image"])		
...
def last_check(attendance_id):
	...
	if exists:
			log,time1=frappe.db.get_value('Employee Checkin',{'device_id': attendance_id}, ["log_type","time"])
			return log,time1	
	return log,time1
...
def get_status(attendance_id='B07ZYJG7KF'):
	employee,name,company,image = get_employee(attendance_id)
	exists=frappe.db.exists('Attendance', {"employee": employee, "attendance_date":nowdate()})
	if exists:
			doc=frappe.db.get_value('Employee Checkin',{'device_id': attendance_id}, "log_type")
			return doc	
	else: 
		return "OUT"
...
def get_checkin_checkout(attendance_id):
	...
	if len(doc)>=1:
		work_hour=calculate_working_hours(doc, check_in_out_type[0], working_hours_calc_type[0])
	else:
		work_hour=0
	return work_hour
...
def set_attendance(doc,attendance_id,type, working_hours=0, shift="Day"):
		...
		if type == "OUT":
			attedance_name=frappe.db.get_value("Attendance",{'employee':employee, "attendance_date": nowdate()}, "name")
