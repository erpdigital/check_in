(function () {
	...
	erpnext.PointOfSale.ItemSelector = class {
		...
		get_item_qty(first_search) {
			...
			if (first_search.startsWith('221')) {
			  ...
			  this.scal_qty = pesokg;
			}
			scal_qty=this.scal_qty;
			
			return this.scal_qty;
		  }
		...
		bind_events() {
            ...
            var customer=['0493543216', '0474632547']
			onscan.attachTo(document, {
				
				onScan: function (sScancode) {

					if ( customer.includes(sScancode)){
						cart.reset_customer_selector();
						cart.customer_field.set_focus();
						cart.set_search_value(sScancode);
					}
					else{
		...
		filter_items(ref) {
			...
			if (search_term) {
				...
				this.get_item_qty(search_term);
				search_term= this.get_search(search_term);
				// memoize
	...
	erpnext.PointOfSale.ItemCart = class {
		...
		set_search_value(value) {
			$(this.customer_field.$input[0]).val(value).trigger("input");
		}
		init_component() {
			this.prepare_dom();
	...
	erpnext.PointOfSale.Controller = class {
		...
		async on_cart_update(args) {
			...
			try {
				...
				if (from_selector)
					{ value = flt(item_row.qty) + flt(qty); }
